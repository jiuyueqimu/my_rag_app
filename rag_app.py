...
def get_llm():
    try:
        ...
        api_key = "sk-你的本地测试Key"  # 本地测试用，部署时使用 st.secrets
...
@st.cache_resource
def get_ocr_engine():
    """延迟初始化 EasyOCR，避免应用启动时立即下载模型"""
    if OCR_AVAILABLE:
        global _reader
        if _reader is None:
            # 初始化 EasyOCR，支持中文和英文，禁用 GPU
            _reader = easyocr.Reader(['ch_sim', 'en'], gpu=False)
        return _reader
    else:
        return None

# 预加载 OCR 模型（在应用启动时触发下载）
if OCR_AVAILABLE:
    try:
        # 使用一个轻量级调用来下载模型（否则首次识别会卡住）
        with st.spinner("正在加载 OCR 模型（首次可能需要 1-2 分钟）..."):
            reader = get_ocr_engine()
            # 用一个小图片触发模型下载（这里只是确保模型下载，不实际识别）
            # 如果还没有模型，会在此处下载
            # 由于没有实际图片，我们用 None 来触发？实际上可以直接调用一次识别，但我们需要一个占位。
            # 更好的办法：在应用启动时，如果环境支持 OCR，我们就创建一个空的识别来触发下载。
            # 注意：EasyOCR 的模型会在第一次调用 readtext 时下载，我们可以在这里调用一次以触发。
            # 但需要一张图片，我们可以创建一个简单的临时文件。
            import numpy as np
            import cv2
            # 创建一个 10x10 的黑色图片（临时）
            ...
            with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp:
                ...
                dummy_path = tmp.name
            # 调用一次 readtext 触发模型下载（但 detail=0 会返回 []，不会实际识别文字）
            reader.readtext(dummy_path, detail=0)
            # 清理临时文件
            os.unlink(dummy_path)
            print("✅ OCR 模型预加载完成")
    except Exception as e:
        # 预加载失败不影响应用启动，后续识别时会再次尝试
        print(f"⚠️ OCR 预加载失败：{e}")
...
def process_uploaded_file(uploaded_file, status_placeholder):
    ...
    try:
        ...
        # ---------- 图片 (PNG, JPG, JPEG, BMP) ----------
            ...
            reader = get_ocr_engine()
            if reader is None:
                status_placeholder.error("❌ 当前环境不支持 OCR，无法识别图片。")
                return False, "OCR 不可用，请使用文本格式文档。"
            status_placeholder.info("⏳ 正在识别图片中的文字...")
            try:
                # EasyOCR 识别
                result = reader.readtext(tmp_path, detail=0)  # detail=0 只返回文字列表
                text = "\n".join(result)
                if not text.strip():
